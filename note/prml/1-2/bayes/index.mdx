...
description: "PRML Chapter 1.2.3"
date: 2025-04-28
subject: "prml"
chapter: "1"
order: 7
...
<Callout variant="definition" title="Định lý Bayes">
...
<MdxImage
src="https://pbs.twimg.com/media/CE5r1ZMUkAAMWIC.png"
alt="meme"
caption="Hình 1: Meme cho ngày vui (thật ra không hài lắm 🥲) (nguồn: xkcd)"
/>
...
Áp dụng sum rule và product rule ([Probability Densities](/note/prml-12/density)) cho biến liên tục, ta có:
...
Đối với Fequentist thì ta sẽ dùng phương pháp **maximum likelihood** (MLE) để tìm giá trị $p(\mathcal{D} \mid \mathbf{w})$ lớn nhất từ đó tìm được $p(\mathbf{w} \mid \mathcal{D})$ lớn nhất (frequentist giả sử rằng $p(\mathbf{w})$ và $p(\mathcal{D})$ là các hằng số, ở góc nhìn của Frequentist, ta sẽ xem $\mathbf{w}$ như là một giá trị mà ta ước lượng được, do đó $p(\mathbf{w})$ là một hằng số) Ta sẽ tìm hiểu phương pháp này ở phần [Gaussian Distribution](/cs4all-vn/prml/chapter1/prob_theory/normal).
