...
description: "PRML Chapter 1.2.1"
date: 2025-04-28
--- 
...
<Callout variant="note">
Giả sử ta có 2 biến liên tục là $x$ và $y$ với $x = g(y)$, tương tự ta có 2 pdf tương ứng là $p(x)$ và $p(y)$. Việc đổi biến từ $y$ sang $x$ sẽ có 1 hiện tượng cực kì hay (hiểu rõ hơn ở bài tập 1.4 [Exercises](../exercises_1/). Hiện tượng này sẽ được áp dụng rất nhiều ở các mô hình Deep Learning ví dụ như Normalizing Flow.)
</Callout>
...
và ngoài ra $P'(x) = p(x)$. Việc chứng minh đạo hàm cdf ta được pdf có thể dùng đến Fundamental Theorem of Calculus [^1]
...
Tương tự như biến ngẫu nhiên rời rạc, sum rule, product rule và định lý Bayes vẫn có thể áp dụng với biến ngẫu nhiên liên tục [^2]. Đặt $x$ và $y$ là hai biến ngẫu nhiên liên tục, ta có:
...
<Callout variant="warning" title="Đây là intuition của mình về mật độ xác suất nên có thể có nhiều sai sót">

<MdxImage
src={explainIm}
alt="explain Image"
caption="Hình 1: Concept của probability density (được mình note thêm một chút) trong sách gốc"
/>
...
[^2]: Việc chứng minh tại sao sum rule, product rule vẫn có thể áp dụng với biến liên tục, ta phải học nâng cao hơn ở môn gọi là Measure Theory (lý thuyết độ đo).